...
for(d in dirIndices){
	...
	if(d==dirIndices[1]){
		plot(timeMidpoints, runmean(avSpeeds, window, endrule="mean"), lwd=2, type='l', xlab="Time (hours)", 
			 ylab=expression(paste("Average speed (",mu,"m per hour)", sep="")), 
			 col=colors[d])
	}else{
	    points(timeMidpoints, runmean(avSpeeds, window, endrule="mean"), lwd=2, type='l', col=colors[d])
	}	
}
